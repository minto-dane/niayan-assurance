...
def generated():
    ...
    for repo in eng.REPOS:
        lines = ['#!/bin/sh', '# SPDX-License-Identifier: BSD-3-Clause',
        ...
        if repo == 'pkgcore':
            ...
            lines.append('timeout --kill-after=5s 610s python3 "$PWD/tests/check_configured_publication.py" --driver "$PWD/build/test-bin/run_generation_publication_tests"')
        ...
        proof = '''# SPDX-License-Identifier: BSD-3-Clause
...
on:
  ...
  workflow_dispatch:
    inputs:
      run_proof:
        description: Run the full component proof at an integration checkpoint
        type: boolean
        default: false
concurrency:
  group: component-${{ github.workflow }}-${{ github.ref }}
...
jobs:
  ...
  proof:
    if: github.event_name == 'workflow_dispatch' && inputs.run_proof
    runs-on: ubuntu-24.04
    timeout-minutes: 120
    container:
      image: debian:13.6-slim@sha256:abc9cb88a5587630d7f915f47b23b0668fe250fbfc6457aa4d52b534c1bbf73f
